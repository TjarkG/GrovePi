...
// for interrupt stuff
constexpr uint8_t total_ports = 9;
...
void processIO()
{
	...
	if (index == 4 && flag == 0)
	{
		...
		else if (cmd[0] == 7)
		{
			...
			const long dur = pulseIn(pin, HIGH, 75000);
			const long RangeCm = dur / 29 / 2;
			b[0] = cmd[0];
			b[1] = RangeCm / 256;
			b[2] = RangeCm % 256;
		...
		else if (cmd[0] == 40)
		{
			if (run_once)
			{
				...
				float *buffer = dht.readTempHum();

				const byte *b1 = reinterpret_cast<byte *>(buffer);
				const byte *b2 = reinterpret_cast<byte *>(buffer + 1);
		...
		{
			const unsigned int state = ledbar[cmd[1] - 2].getBits();
			b[0] = cmd[0];
			b[1] = state & 0xFF;
			b[2] = state >> 8;
		}
		...
		{
			const int pin = cmd[2];
			const int reads = 4 * cmd[3]; // 1000/250 * cmd[3]
		...
		{
			constexpr uint8_t data[] = {0x00, 0x00, 0x00, 0x00};
			fourdigit[cmd[1] - 2].setSegments(
				data, 4, 0); // setSegments(segments[], length, position)
		}
		...
		else if (cmd[0] == 93)
		{
			...
			} else
			{
				...
				const byte num_leds = rgbled[cmd[1] - 2].getNumLeds();
	...
	} else if (cmd[0] == isr_set_cmd)
	{
		if (run_once == 1)
		{
			const uint8_t pin = cmd[1] & 0x0f;                   // 1st 4 bits to determine pin (D2->D8)
			...
			{
				PcInt::attachInterrupt<uint8_t>(pin, isr_handler, const_cast<uint8_t *>(&pins[pin]), interrupt_mode,
				                                true);
			} else if (ftype == COUNT_LOW_DURATION)
			{
				pulse_counter[pin] = Pulse_counter({0, 0, 0});
				PcInt::attachInterrupt<uint8_t>(pin, isr_handler, const_cast<uint8_t *>(&pins[pin]), CHANGE, false);
			}
			run_once = 0;
		}
	...
	} else if (cmd[0] == encoder_read_cmd)
	{
		...
		{
			const uint8_t pin = cmd[1];
			const int32_t value = ge[pin].value;
			Serial.print(value);
			Serial.print("\n");
...
void sendData()
{
	...
	{
		if (cmd[0] == 1)
			Wire.write(const_cast<byte *>(b), 2);
		if (cmd[0] == 3 || cmd[0] == 7 || cmd[0] == 56)
			Wire.write(const_cast<byte *>(b), 3);
		if (cmd[0] == 8 || cmd[0] == 20)
			Wire.write(const_cast<byte *>(b), 4);
		if (cmd[0] == 30)
			Wire.write(const_cast<byte *>(b), 9);
		if (cmd[0] == 40)
			Wire.write(const_cast<byte *>(dht_b), 9);
		if (cmd[0] == isr_read_cmd)
			Wire.write(const_cast<byte *>(b), 5);
		if (cmd[0] == isr_active_cmd)
			Wire.write(const_cast<byte *>(b), 3);
		if (cmd[0] == ir_read_cmd)
		{
			Wire.write(const_cast<byte *>(b), 8);
			b[0] = 0;
		}
...
void isr_handler(uint8_t *userdata, bool newstate)
{
	...
	} else if (func_type[pin] == COUNT_LOW_DURATION)
	{
		...
		{
			const unsigned long duration = (pulse_end - pulse_start) / 1000; // to get ms
